...
class Ibex(Cluster):
    ...
    ***
    ...
    def __init__(self, ntask_max=100, slurm_args="",  **kwargs):
        ...
        self.submit_to = 'batch'
        self.acceptable_partitions = ['batch']
        self._partitions = {'batch': 40}
    ...
    def check(self):
        ...
        assert(self.partition in self.acceptable_partitions), \
            f"Cluster partition name must match {self._partitions}"

        assert(self.submit_to in self.acceptable_partitions), \
...
def modify_run_call_single_proc(run_call):
    ...
    for part in run_call.split(" "):
        if "--array" in part:
            ...
            run_call = run_call.replace(part, "--ntasks=8")
